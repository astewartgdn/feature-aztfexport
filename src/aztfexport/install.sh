...
echo "Activating feature 'aztfexport'"

# Clean up
rm -rf /var/lib/apt/lists/*

AZTFE_VERSION=${VERSION:-"latest"}
AZTFE_EXTENSIONS=${EXTENSIONS}
AZTFE_INSTALLBICEP=${INSTALLBICEP:-false}
...
install_using_apt() {
    ...
    echo "deb [arch=${architecture} signed-by=/usr/share/keyrings/microsoft-archive-keyring.gpg] https://packages.microsoft.com/repos/aztfexport/ ${VERSION_CODENAME} main" > /etc/apt/sources.list.d/aztfexport.list
    apt-get update

    if [ "${AZTFE_VERSION}" = "latest" ] || [ "${AZTFE_VERSION}" = "lts" ] || [ "${AZTFE_VERSION}" = "stable" ]; then
        # Empty, meaning grab the "latest" in the apt repo
        AZTFE_VERSION=""
    else
        # Sets AZTFE_VERSION to our desired version, if match found.
        apt_cache_version_soft_match AZTFE_VERSION "aztfexport" false
        if [ "$?" != 0 ]; then
            return 1
        fi
    fi

    if ! (apt-get install -yq aztfexport${AZTFE_VERSION}); then
        rm -f /etc/apt/sources.list.d/aztfexport.list
        return 1
    fi
}
...
install_with_complete_python_installation() {
    ...
    set +e
        ${pipx_bin} install --pip-args '--no-cache-dir --force-reinstall' -f aztfexport${ver}

        # Fail gracefully
        if [ "$?" != 0 ]; then
            echo "Could not install aztfexport${ver} via pip3"
            rm -rf /tmp/pip-tmp
            return 1
        fi
    set -e
...
CACHED_AZURE_VERSION="${AZTFE_VERSION}" # In case we need to fallback to pip and the apt path has modified the AZTFE_VERSION variable.
...
if [ "${use_pip}" = "true" ]; then
    AZTFE_VERSION=${CACHED_AZURE_VERSION}
    install_using_pip_strategy
    ...
    if [ "$?" != 0 ]; then
        ...
        apt-cache madison aztfexport | awk -F"|" '{print $2}' | grep -oP '^(.+:)?\K.+'
        exit 1
    fi
fi

# If Azure CLI extensions are requested, loop through and install 
if [ ${#AZTFE_EXTENSIONS[@]} -gt 0 ]; then
    echo "Installing Azure CLI extensions: ${AZTFE_EXTENSIONS}"
    extensions=(`echo ${AZTFE_EXTENSIONS} | tr ',' ' '`)
    for i in "${extensions[@]}"
    do
        echo "Installing ${i}"
...
if [ "${AZTFE_INSTALLBICEP}" = "true" ]; then
    # Install dependencies
